 ...
 * L'affichage des coordonnées peut se faire suivant une des projections valides à cet endroit de la carte
 */
L.Marker.include({
	coordinates: function(id) {
		...
		if (el) {
			var v = el.innerHTML || el.value;
			if (v) {
				var cs = JSON.parse(v).coordinates;
				if (cs)
	...
	_displayCoord: function() {
		...
		if (selectEL) {
			...
			for (c in L.CRS)
				if (L.CRS[c].name &&
					typeof L.CRS[c] == 'object' && // Evite les functions
