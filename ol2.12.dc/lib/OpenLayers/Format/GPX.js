...
//FCM//     point_features.push(new OpenLayers.Geometry.Point(points[i].getAttribute("lon"), points[i].getAttribute("lat")));
/*FCM*/		var p = new OpenLayers.Geometry.Point(points[i].getAttribute("lon"), points[i].getAttribute("lat")); 
/*FCM*/		var ele = points[i].getElementsByTagName("ele"); 
/*FCM*/		if (ele && ele.length>0)
/*FCM*/			p.z = parseFloat(ele[0].textContent); 
/*FCM*/		var time = points[i].getElementsByTagName("time"); 
/*FCM*/		if (time && time.length>0)
/*FCM*/			p.time = time[0].textContent; 
/*FCM*/		point_features.push(p);
        }
...
/*FCM*/	if (point.z) {
/*FCM*/        var ele = this.createElementNSPlus('gpx:ele');
/*FCM*/	        ele.appendChild(this.createTextNode(point.z));
/*FCM*/        	node.appendChild(ele);
/*FCM*/	}
/*FCM*/	if (point.time) {
/*FCM*/        var time = this.createElementNSPlus('gpx:time');
/*FCM*/	        time.appendChild(this.createTextNode(point.time));
/*FCM*/        	node.appendChild(time);
/*FCM*/	}
        return node;
    },
