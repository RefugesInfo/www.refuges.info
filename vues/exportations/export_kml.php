<?php
	header("Content-disposition: attachment; filename='$vue->nom_fichier_export.kml");
	header("Content-Type: text/xml; charset=$vue->content_type");
	header("Content-Transfer-Encoding: binary");
	header("Pragma: no-cache");
	header("Expires: 0");
?>
<?='<?'?>xml version="1.0" encoding="<?php$vue->content_type?>"?>
<kml xmlns="http://earth.google.com/kml/2.1">
<Document>
	<name><?=$vue->nom_fichier_export?>.kml</name>
	<description><?=$vue->description?></description>
	
	<!-- Liste des Styles -->
	<open>1</open>
	<?if ($vue->pois) foreach ($vue->pois AS $type => $t) {?>
	...
	<!-- Liste des POINTS -->
	<?if ($vue->pois) foreach ($vue->pois AS $type) {?>
