 ...
 ***************************************************************************/
/*
	Mod added "manually", looks I became the "patch" program
	png visual confirmation system : (c) phpBB Group, 2003 : All Rights Reserved

*/
/* par sly, protection anti-robot
 code statique facile à comprendre pour un humain, mais dur pour un robot
 ceci est la réponse à la question donnée plus bas (à coté du texte BEGIN of added mod to visual securty against spam bots)
...
//$interdit = Array ('www.flyers-store.com','www.flyers-shop.com','www.thrashers-shop.com','www.canucks-shop.com','www.sabres-shop.com');
// 15/04/2011 Dominique : extension à tout le championnat de hockey sur glace américain
$interdit = Array (
	'nhl', // National Hockey League
	'viagra', // On ne sait jamais
);
foreach (Array (
	'flyers', 'thrashers', 'canucks', 'sabres', 'capitals', 'rangers',
	'blackhawks', 'sharks', 'kings', 'gear', 'canadiens', 'wings',
	'coyotes', 'duks', 'predator', 'lightning', 'penguins', 'bruins',
) AS $team)
	foreach (Array ('shop', 'store', 'printing-store') AS $store)
		$interdit [] = "$team-$store";


/* Fichtre, des spammeurs arrivent à passer, tentons de comprendre comment :
plus utile pour l'instant, j'ai compris, c'est un vrai humain qui lit le français
...
//  sly's hack : réponse à la question pour les anonymes
$error_msg = '';

// de façon assez incompréhensible, cette page arrive à être exploitée par les spammeurs
// malgré mon intégration semi-propre d'une sécurité anti-spam
// surement un BUG phpBB, j'abandonne donc le catcha image pour un captcha avec question en français
if ( (($_POST['mode']=="reply") || ($_POST['mode']=="newtopic")) && ($_POST['confirm_code']!=$clef_visuel) && !$userdata['session_logged_in'])
	$error_msg="Clef de confirmation incorrect";

// fin du hack

// sly's hack : une fonction commune avec le site refuges.info bloque des FQDN de certains internautes
// en date du 15/04/2011 cette fonction ne bloque plus personne

if (bloquage_internaute() && !$userdata['session_logged_in'])
	$error_msg="Erreur, votre accès a été supprimé voir <a href=\"http://forum.refuges.info/viewtopic.php?t=671\">ici sur le forum</a><br/> Néamoins, la création d'un compte vous permettra de passer";
...
// Hack anti spam
				...
				foreach ($interdit AS $i)
					if ($t = stristr ($message, $i))
						$spam = true;
				if ($spam) {
					$error_msg="pas de spam sur ce site, merci";
					file_put_contents ('spam.log', $_SERVER['REMOTE_ADDR'] .date (' / Y-m-d H:i') ."\n$message\n", FILE_APPEND);
				}
				else
// Fin hack anti spam
	...
	if ( $error_msg == '' )
	{
		...
		//sly's hack pour prévenir ceux qui l'aurront choisi d'un nouveau topic 25/03/2010
		if ($mode == 'newtopic')
		{
			// si c'est sur le forum de modération (id 7), il faut être modérateur (196 c'est le numéro du groupe)
			if ($HTTP_POST_VARS['f']==7)
			{
			  $table=",phpbb_user_group";
			  $conditions_join="phpbb_users.user_id=phpbb_user_group.user_id AND";
			  $condition_groupe="AND phpbb_user_group.group_id=196";
			}
...
Un nouveau sujet a été créé sur le forum de refuges.info
##############################################################
De : ".$userdata['username']."
Titre : ".$HTTP_POST_VARS['subject']."
Message : 
".bbcode2txt($HTTP_POST_VARS['message'])."
...
de reçevoir une alerte à chaque nouveau sujet posté sur le forum de refuges.info
Si vous ne souhaitez plus reçevoir cette alerte, vous pouvez vous connectez sur le forum,
aller dans votre profil et choisir \"Reçevoir un e-mail à chaque nouveau sujet\" -> non

En cas de problème, venez en parler sur le forum
");
				mail($user_a_prevenir->user_email,$titre_email,$contenu_message,"From: existe-pas@refuges.info\r\nContent-Type: text/plain;charset=\"utf-8\"\r\n","-f existe-pas@refuges.info");
			}
		}
...
/********
BEGIN
of added mod to visual securty against spam bots
********/
$confirm_image = '';
...
/**END of added mod to visual securty against spam bots **/	
